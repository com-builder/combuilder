...
class {{Name}}Model{{Item}} extends AdminModel
{
     ...
     * same name. If no value for `$name` is provided, it will be initialized
     * to "{{Item}}s". If no value for `$prefix` is provided, it will be
     * initialized to "{{Name}}Table".
     *
     ...
     *                             table.
     *
     * @throws    \Exception  When the requested `Table`instance couldn't be
     *                        created.
     *
     * @return    \Joomla\CMS\Table\Table  A `Table` instance representing the
     *                                     "{{Item}}s" table.
     */
    ...
    /**
     * Fetch the current form data from the user state (fallback to the
     * database).
     *
    ...
    protected function loadFormData()
    {
        ...
        // If there is no user state for the form, fetch the "{{Item}}" being
        // edited
