...
class {{Name}}Model{{Item}}s extends ListModel {
    /**
     * Supplement the default constructor to configure the model's filter
     * fields.
     *
     ...
     *
     * The query will be pre-configured to contain only the `id`, `published`
     * and `name` columns. Additionally, if the `checked_out` column is
     * non-NULL, then `editor` will contain the name of the user that has the
     * record checked-out. Otherwise, `editor` is NULL.
     *
     * Depending on the active filters and ordering configuration in the user
     * state, the pre-configured functionality of the resulting query can
     * differ in the following ways:
     *
     * - `filter.search`:     If the user state contains this key, the result
     *                        set will be filtered to contain results whose
     *                        `name` column match the provided value surrounded
     *                        by wild{{item}}s on either side. Otherwise, the
     *                        result set is not filtered by `name`. This filter
     *                        is case-insensitive.
     * - `filter.published`:  If the user state contains this key, the result
     *                        set will be filtered to contain results whose
     *                        `published` column match the provided value
     *                        exactly. Otherwise, the result set will contain
     *                        only published and unpublished records
     *                        (excluding trashed records).
     * - `list.ordering`:     If the user state contains this key, the results
     *                        set will be ordered based on the value of the
     *                        specified column. Otherwise, the result set is
     *                        ordered by the `id` column.
     * - `list.direction`:    If the user state contains this key, the results
     *                        set will be ordered based on the specified
     *                        direction (ascending or descending). Otherwise,
     *                        the result set is ordered ascending.
     *
     * The user state key `list.ordering` must contain the prefix '{{item}}s.'
     * before  the provided column name so that there is no ambiguity between
     * the "{{Item}}s" and the "Users" table. This is because these two tables
     * are joined to fetch the name of the "User" represented by the ID stored
     * in `{{item}}s.checked_out`.
     *
    ...
    protected function getListQuery()
    {
        ...
        if (is_numeric($published))
        {
            ...
            $query->where(
                $db->quoteName('{{item}}s.published').' = '.intval($published));
        }
        else
        {
